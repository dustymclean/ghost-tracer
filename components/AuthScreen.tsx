    ...
    <div className="min-h-screen bg-[#050508] flex items-center justify-center p-6 relative">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
           <div className="inline-block p-4 rounded-3xl bg-black border border-[#1c1c2e] mb-6 shadow-[0_0_30px_rgba(0,240,255,0.1)]">
        ...
        <div className="bg-[#0a0a0f] border border-[#1c1c2e] p-10 rounded-[2.5rem] shadow-2xl">
          <div className="flex items-center gap-2 text-[10px] font-black text-[#00f0ff] uppercase tracking-widest mb-8"><Lock className="w-3 h-3" /> Authorization Required</div>
          <div className="space-y-4">
            <input type="email" placeholder="OPERATOR_EMAIL" className="w-full bg-black border border-[#1c1c2e] p-5 rounded-2xl text-white font-mono focus:border-[#00f0ff] outline-none" onChange={(e) => setEmail(e.target.value)} />
            <input type="password" placeholder="SECRET_ACCESS_CODE" className="w-full bg-black border border-[#1c1c2e] p-5 rounded-2xl text-white font-mono focus:border-[#00f0ff] outline-none" onChange={(e) => setPassword(e.target.value)} />
          </div>
          {message && <p className="mt-4 text-[10px] font-bold text-red-500 uppercase text-center">{message}</p>}
