...
export const generateOsintReport = async (query: string, apiKey: string): Promise<OsintReport> => {
  ...
  const systemPrompt = `You are GhostTrace, an elite forensic OSINT analyst. 
  Generate a detailed intelligence report for: "${query}". 
  You MUST return ONLY a raw JSON object. Do not include markdown code blocks.
  ...
  try {
    ...
    // Rigorous cleaning to ensure valid JSON parsing
    const cleanJson = rawText.replace(/```json|```/g, "").trim();
    const parsed = JSON.parse(cleanJson);

    // Map string to Enum to satisfy the OsintReport interface
