    ...
    <div className="min-h-screen bg-[#050508] text-white flex flex-col font-sans overflow-x-hidden selection:bg-[#00f0ff] selection:text-black">
      <nav className="h-16 border-b border-[#1c1c2e] bg-[#0a0a0f]/90 backdrop-blur-md flex items-center justify-between px-8 z-50 sticky top-0">
        ...
        <div className="flex items-center gap-8">
          <button onClick={() => setShowManual(true)} className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-[#00f0ff] uppercase tracking-widest"><BookOpen className="w-4 h-4" /> Manual</button>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl">
          <div className="w-full max-w-2xl bg-[#0a0a0f] border border-[#1c1c2e] p-10 rounded-3xl shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-black uppercase tracking-tighter flex items-center gap-3"><ShieldCheck className="w-7 h-7 text-[#00f0ff]" /> Operations Protocol</h3>
              <button onClick={() => setShowManual(false)}><X className="w-8 h-8 text-gray-500 hover:text-white" /></button>
            </div>
            <div className="space-y-6 text-sm text-gray-400 leading-relaxed">
              <p><strong>01. Logic:</strong> Ghost Tracer leverages Claude 3.5 via OpenRouter to conduct forensic corporate research.</p>
              <p><strong>02. Sovereignty:</strong> This is a BYOK node. Your credentials stay in local storage; your searches stay in your private ledger.</p>
              <p><strong>03. Use Case:</strong> Investigative auditing, due diligence, and ethical watchdog activity.</p>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="w-full max-w-md bg-[#0a0a0f] border border-[#1c1c2e] p-8 rounded-2xl shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black uppercase tracking-widest flex items-center gap-2"><Key className="w-5 h-5 text-[#00f0ff]" /> OpenRouter Key</h3>
            ...
            <div className="bg-[#1c1c2e]/30 p-5 rounded-xl mb-6 text-xs text-gray-300">
               <p className="mb-4 text-white font-bold">Instructional Note:</p>
               <p className="mb-4">Obtain your API Key from OpenRouter to power this machine. Dusty makes zero profit on these keys.</p>
               <a href="https://openrouter.ai/keys" target="_blank" className="text-[#00f0ff] font-bold hover:underline flex items-center gap-2">Get OpenRouter Key <ExternalLink className="w-3 h-3" /></a>
            </div>
            <input type="password" placeholder="sk-or-..." className="w-full bg-black border border-[#1c1c2e] p-4 rounded-xl mb-6 text-[#00f0ff] font-mono outline-none" value={userApiKey} onChange={(e) => setUserApiKey(e.target.value)} />
          ...
          <div className="w-full pb-20 px-8">
             {searchState.isLoading ? (
               ...
               <div className="py-32 flex flex-col items-center text-center max-w-xl mx-auto">
                  ...
                  <button onClick={() => setView('home')} className="px-10 py-4 bg-white text-black font-black uppercase rounded-full">Reset Console</button>
               </div>
